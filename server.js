...
app.get("/reserva/all", async (req, res) => {
    ...
    try {
        ...
        if (
            ...
            result.length > 0 &&
            result[0][0] &&
            result[0][0].id_reserva !== undefined
        ) {
            res.json(result[0]); // Envia a reserva como resposta
        } else {
    ...
    try {
        await pool.query(
            ...
            VALUES (${id_reserva}, ${cpf_resp}, ${id_service_plan}, ${id_pag}, 
                ...
                date_check_in
            )}', '${formatarDataParaBanco(date_check_out)}')`
        );
...
app.put("/reserva/:id_reserva", async (req, res) => {
    ...
    const { cpf_resp, id_service_plan, id_pag, date_check_in, date_check_out } =
        req.body;
    ...
    try {
        ...
        if (
            !(
                ...
                reservaExistente[0][0] &&
                reservaExistente[0][0].id_reserva !== undefined
            )
        ) {
        ...
        await pool.query(
            `UPDATE reserva 
             SET cpf_resp = ${cpf_resp}, id_service_plan = ${id_service_plan}, id_pag = ${id_pag}, 
                 date_check_in = '${formatarDataParaBanco(
                date_check_in
            )}', date_check_out = '${formatarDataParaBanco(
                date_check_out
            )}'
             WHERE id_reserva = ${id_reserva}`
        );
...
// Rota para pesquisar reserva pelo número de reserva
app.get("/hospede/all", async (req, res) => {
...
app.post("/hospede", async (req, res) => {
    ...
    try {
        await pool.query(
            ...
            VALUES (${cpf_resp}, '${nome}', ${genero}, '${formatarDataParaBanco(
                data_nasc
            )}')`
        );
        const result = await pool.query(`SELECT 
...
app.put("/hospede/:cpf_resp", async (req, res) => {
    ...
    try {
        // Verifica se a reserva com o ID especificado existe no banco de dados
        // Se não existir, retorna um erro 404 (Not Found)
        const hospedeExistente = await pool.query(
            `SELECT * FROM hospede WHERE cpf_resp = ${cpf_resp}`
        );
        if (
            !(
                hospedeExistente &&
                hospedeExistente.length > 0 &&
                hospedeExistente[0][0] &&
                hospedeExistente[0][0].cpf_resp !== undefined
            )
        ) {
        ...
        await pool.query(
            `UPDATE hospede 
             SET nome = '${nome}', data_nasc = '${formatarDataParaBanco(
                data_nasc
            )}', genero = ${genero}
             where cpf_resp = ${cpf_resp}`
        );
...
app.post("/quarto", async (req, res) => {
    const { num_quarto, status_quarto_ocupado, status_quarto_livre, id_reserva } =
        req.body;
    try {
        var status;
        ...
        await pool.query(
            ...
            VALUES (${num_quarto}, '${status_quarto_ocupado == true ? 1 : 2
            }', ${id_reserva})`
        );
...
app.put("/quarto/:numQuarto", async (req, res) => {
    const { num_quarto, status_quarto_ocupado, id_reserva, id_reserva_go } =
        req.body;
    const reservaInicial = id_reserva;

    console.log("aaaaaaaaaaaaaaaaaaaaaaaa " + status_quarto_ocupado);
    ...
    try {
        ...
        if (
            !(
                quartoExistente &&
                quartoExistente.length > 0 &&
                quartoExistente[0][0] &&
                quartoExistente[0][0].num_quarto !== undefined
            )
        ) {
...
app.delete("/quarto/:num_quarto", async (req, res) => {
    ...
    try {
        ...
        if (quarto.num_quarto != undefined) {
            await pool.query(`DELETE FROM quarto WHERE num_quarto = ${num_quarto}`);
            const result = await pool.query(
                `select quarto.num_quarto, 
                status_quarto, 
                quarto.id_reserva, hospede.cpf_resp,
...
app.put("/animal/:id_animal", async (req, res) => {
    ...
    try {
        ...
        if (
            !(
                animalExistente &&
                animalExistente.length > 0 &&
                animalExistente[0][0] &&
                animalExistente[0][0].id_animal !== undefined
            )
        ) {
            return res.status(404).json({ message: "Quarto não encontrada" });
        }
...
app.delete("/animal/:id_animal", async (req, res) => {
    ...
    try {
        ...
        if (animal.id_animal != undefined) {
            await pool.query(`DELETE FROM animal WHERE id_animal = ${id_animal}`);
            const result = await pool.query(
                `select 
                id_animal, id_reserva, nom_animal, id_raca, pedigre
                from animal
...
app.get("/tiposervico", async (req, res) => {
    try {
        ...
        const tiposServico = await pool.query(
            "select id_service_plan, nom_service from service_plan"
        );
        ...
        res
            .status(500)
            .json({ message: "Erro ao buscar opções do tipo de serviço" });
    }
});
...
app.get("/tipopagamento", async (req, res) => {
    try {
        // Consulta ao banco de dados para obter os valores possíveis para tipo_servico
        const tiposPagamento = await pool.query(
            "select id_pag, nom_pag from pagamento"
        );
        ...
        res
            .status(500)
            .json({ message: "Erro ao buscar opções do tipo de pagamento" });
    }
});
...
app.get("/genero", async (req, res) => {
    try {
        // Consulta ao banco de dados para obter os valores possíveis para tipo_servico
        const tiposGenero = await pool.query(
            "select id_gen, nom_genero from genero"
        );
        ...
        res
            .status(500)
            .json({ message: "Erro ao buscar opções do tipo de genero" });
    }
});
